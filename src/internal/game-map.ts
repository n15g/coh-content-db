...
export class GameMap implements IGameMap {
  ...
  public load(data: IGameMapData) {
    ...
    if (data.vidiotMaps) {
      for (const vidiotMapData of data.vidiotMaps) {
        this.vidiotMaps?.push(new VidiotMap(this.serverGroup, this).load(vidiotMapData))
      }
    }
  }
}
...
export class VidiotMap implements IVidiotMap {
  ...
  public load(data: IVidiotMapData): VidiotMap {
    ...
    if (data.pointsOfInterest) {
      for (const poiData of data.pointsOfInterest) this.pointsOfInterest.push(new VidiotMapPoi(this.serverGroup, this.map, this).load(poiData))
    }
